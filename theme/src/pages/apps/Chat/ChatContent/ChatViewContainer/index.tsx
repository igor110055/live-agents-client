...
import {ConnectionObj, MessageDataObj, MessageObj,} from "../../../../../types/models/apps/Chat";
...
const ChatViewContainer: React.FC<MessagesScreenProps> = ({ selectedUser }) => {
  ...
  const {userMessages} = useSelector<AppState, AppState["chatApp"]>(({chatApp}) => chatApp);
    ...
    >
      ...
      {userMessages && user ? (
        <ScrollbarWrapper ref={_scrollBarRef}>
          <MessagesList
            userMessages={userMessages as MessageObj}
            authUser={user}
            selectedUser={selectedUser}
